...
function CreatePostAuthenticated(user)
{ 
...
//  Fetch page access token for sending post requests on the default page id provided
    window.FB.api(
        `/${pageID}?fields=access_token&access_token=${user.user.accessToken}`,function(response)
        {
...
export default function CreatePost({userContext})
{
  ...
  // let post= (user.auth && !user.pageID)?<CreatePostAuthenticated user={user} updatelogin={updatelogin} />:(user.pageID)?navigate('/createcomment'):<AuthFailed/>;
    let post= user.auth?<CreatePostAuthenticated user={user} />:<AuthFailed/>;
    return (post);
}